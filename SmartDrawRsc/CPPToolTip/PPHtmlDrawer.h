...
//	#include "StdString.h"
//	#ifdef _UNICODE
//	#define CString	CStdStringW	//non-MFC program UNICODE
//	#else
//	#define CString	CStdStringA	//non-MFC program ANSI
//	#endif
//#endif
...
class ON_EXT_SMARTDRAWRSC CPPHtmlDrawer
{
// Construction
public:
	CPPHtmlDrawer();
...
#pragma pack(1)
	typedef struct _STRUCT_TAGPROP
	{
		...
		CString strTagName;	// The type of the hyperlink
	} STRUCT_TAGPROP;
#pragma pack()
...
//	POINT m_ptOutput; //Output coordinates
	...
	CString m_csHtmlText; //String to output
...
//	int m_nLineDescent;
	...
	//Map of the colors by name
	typedef std::map<CString, STRUCT_TAGPROP> mapTags;
...
protected:
	...
	DWORD GetTagFromList(CString sTagName, CString & strFullName, BOOL & bCloseTag); //Get tag from the list
	...
	SIZE DrawHtmlTable(CString & sTable, LPCRECT lpRect); //Draws the HTML table on device context or gets the size of the output area.
//	SIZE DrawHtmlTableRow(CString & sRow, LPCRECT lpRect, vecCol & row); //Draws the HTML row of the table
	void DrawHtmlTableRow(CString & sRow, LPCRECT lpRect, STRUCT_TABLE & st, int nRow);
	void DrawHtmlTableCell(CString & sCell, LPCRECT lpRect, STRUCT_CELL & sc); //Draws the HTML cell of the table
	SIZE DrawHtmlString(CString & sHtml, LPCRECT lpRect); //Draws the HTML string on device context or gets the size of the output area.
...
//public:
	...
	CString SearchNextTag(CString & str, CString & strTag, int & nIndex); //Search next tag
	BOOL SearchTag(CString & str, int & nIndex, CString strTag); //Search begin of the specified tag

	CString GetTagBody(CString & str, int & nIndex); //Gets a name of tag and the parameters of tag
	CString SplitTag(CString & sTag); //Split a tag to the tag's name and parameters
	CString GetNextProperty(CString & str, int & nIndex, CString & sProp); //Gets next property


	CString SearchPropertyOfTag(CString & str, int & nIndex); //Search a name or a property of a tag
	SIZE  AnalyseCellParam(CString & sTag, _STRUCT_CHANGESTYLE & cs, BOOL bTable);
	...
	SIZE  GetTableDimensions(CString & sTable); //Gets dimensions of the table
	void  SearchEndOfTable(CString & str, int & nIndex); //Searching end of the table
	void  SearchEndOfRow(CString & str, int & nIndex); //Searching end of the row
	void  SearchEndOfCell(CString & str, int & nIndex); //Searching end of the cell
	...
	CString GetWordWrap(CString & str, int nMaxSize, int & nRealSize);
	...
	//Drawing
	void DrawBackgroundImage(HDC hDC, int nDestX, int nDestY, int nWidth, int nHeight, CString strNameImage);

	int GetCountOfChars(CString str, TCHAR tchar = _T(' ')); //Gets counts of chars
};
