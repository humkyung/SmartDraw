...
#include "CableTrayDrawingOption.h"
...
int CCableTrayDrawingImpl::Write(const STRING_T& sOutputFilePath)
{
	...
	if(ofile.is_open())
	{
		...
		ofile << _T("<DIMSTYLE>|EXT_DIMSTYLE,3,2,1.6,0,0,1,0,1.5,1.50000,3,5,5") << std::endl;
...
int __stdcall Annotate(LPCTSTR pOutputFilePath , LPCTSTR pDraFilePath , LPCTSTR pPrjName , LPCTSTR pPrjFolderPath)
{
	...
	{
		STRING_T sDatabaseFilePath(pPrjFolderPath + STRING_T(_T("\\Setting\\")) + pPrjName + STRING_T(_T(".mdb")));
		STRING_T sPrjFilePath(pPrjFolderPath + STRING_T(_T("\\Setting\\")) + pPrjName + STRING_T(_T(".prj")));
		STRING_T sIniFilePath;
		...
		if(ERROR_SUCCESS == pDraFile->Parse())
		{
...

			{
				...
				oDrawing->Annotate(pOutputFilePath , sHsrFilePath.c_str() , pDraFile.get());
			}
		}
