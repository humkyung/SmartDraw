...
Private  void getPatternInfo(MSElementDescr* pElmDescr,int elementcount)
{
    ...
    while (pDescr != NULL)
    {
	    ...
	    else if( mdlElement_getType(&pDescr->el) == LINE_ELM  && ((elementcount == 6 && iCnt == 2) 
		...
		if( SUCCESS == mdlLinear_extract(points,&nVertices,&pDescr->el,MASTERFILE))
            ...
            /// up to here
            /// get enclouse points
            else if(mdlElement_getType(&pDescr->el) == LINE_STRING_ELM  && iCnt == 1)
            {
		if( SUCCESS == mdlLinear_extract(points,&nVertices,&pDescr->el,MASTERFILE))
                {
		    g_linevertices[0] = points[0];
                    g_linevertices[1] = points[1];
                    g_linevertices[2] = points[2];
                    g_linevertices[3] = points[3];
                }
            }
            /// up to here
			...
			||( strcmp(infoP->cPatternName,PATTERN_2) == 0 && 
			 ...
			 ||( strcmp(infoP->cPatternName,PATTERN_7) == 0 && 
                         ...
                         ||( strcmp(infoP->cPatternName,PATTERN_8) == 0 && 
  		                     ( g_elementpos == ELM_POS_2 || g_elementpos == ELM_POS_5)))                                       
...
Private void   dynamicRotation(Dpoint3d *pPoint, int view )
{
    ...
    int			elementcount;
    double		angle = 0.0,tempangle = 0.0,minangle = -1;
    ULong		filePos;
	...
	if(!mdlSelect_isActive())
				...
				|| strcmpi(info.cPatternName,PATTERN_8) == 0 )
		{
			...
			if(TRUE == isRotateBy90)
			{
				angle *= 180/3.141592;
				if(angle < 0) angle += 360;
				if(angle > 360) angle -= 360;

				offset[0] = fabs(angle - 0);
				offset[1] = fabs(angle - 90);
				offset[2] = fabs(angle - 180);
				offset[3] = fabs(angle - 270);
				offset[4] = fabs(angle - 360);

				index = 0;
				minangle = offset[index];
				for(i = 1;i < 5;++i)
				{
					if(offset[i] < minangle)
					{
						minangle = offset[i];
						index = i;
					}
		    	...
		    	info._angle = index*90*3.141592/180;
			}
			else
			{
				info._angle = angle;
...
{
    int			modifyFlags = MODIFY_ORIG	;
    int			fileNum,elementcount,i,index;
    double		angle = 0.0;
...
cmdNumber CMD_TECHSUN_TAGROTATE
{
...
/*****************************************************************************************
//Function : rotatePattern By 90
//Desc     : Rotate cell element By 90 - PATTERNS
//Return   : None
//****************************************************************************************/
cmdName void    rotatePatternBy90
(
    char        *unparsedP
)
cmdNumber CMD_TECHSUN_TAGROTATEBY90
{
    StatusInt   status = SUCCESS;
    int		gFilePos = 0L;
    ...
    mdlLocate_init();
    mdlLocate_normal ();
    mdlState_startModifyCommand (rotatePatternBy90,
				 rotatePattern_accept,
				 dynRotation, 
                                 NULL, 
                                 NULL,
				 COMMANDID_RotatePattern_R90, 
                                 PROMPTID_CustomAcceptReject,
				 TRUE, 
                                 1);
...

{
    ...
    {
	/* Dont move following elements(leader - Arrows)*/
        if( /* Pattern -2 */
	    (strcmp(info->cPatternName,PATTERN_2) == 0 && 
		    (g_elementpos == ELM_POS_2||(g_elementpos == ELM_POS_4 && 
	    ...
	    /* Pattern -7 */                                                         
            ...
            /* Pattern -8 */        
            (strcmp(info->cPatternName,PATTERN_8) == 0 && (g_elementpos == ELM_POS_3 || g_elementpos == ELM_POS_4)) 
          )  
	{  
            g_elementpos ++;
	    return MODIFY_STATUS_REPLACE;
	...
	/* up to here */
	   ...
	   // strcmp(info->cPatternName,PATTERN_4) == 0 ||
        ...
        /*
        sprintf(msg , "%s : %d,%d" , info->cPatternName , g_elementpos , iElmType);
	...
	if( strcmp(info->cPatternName,PATTERN_3) != 0 )
        ...
        if((strcmp(info->cPatternName,PATTERN_2) == 0 && (g_elementpos == ELM_POS_2 || g_elementpos == ELM_POS_3)))
	{
        ...
        if(strcmp(info->cPatternName,PATTERN_7) == 0 && g_elementpos == ELM_POS_2) // Text element - extract the range information
	{
        ...
        /* Pattern -8*/
        if(strcmp(info->cPatternName,PATTERN_8) == 0  && g_elementpos == ELM_POS_2) // enclosure shape element
	{
	    mdlLinear_extract(points,NULL,el,MASTERFILE);	
            g_roundrect[0] = points[0];
            g_roundrect[1] = points[1];
            g_roundrect[2] = points[2];
            g_roundrect[3] = points[3];
        }
...
)
{
    ...
    if( ((strcmp(info->cPatternName,PATTERN_2) ==0 || strcmp(info->cPatternName,PATTERN_3) ==0) 
			&& g_elementpos == ELM_POS_2)
                        || (strcmp(info->cPatternName,PATTERN_7) ==0 && g_elementpos == ELM_POS_3) 
			|| (strcmp(info->cPatternName,PATTERN_8) ==0 && g_elementpos == ELM_POS_3))                                     
    {
...
//    mdlSystem_enterDebug();
    ...
    */
    ...
    mdlDialog_open (NULL, DIALOGID_PWTagEditor);
