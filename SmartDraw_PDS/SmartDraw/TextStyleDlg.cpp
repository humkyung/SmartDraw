...
BOOL CTextStyleDlg::OnInitDialog()
{
...

	{
		HICON hIcon = (HICON) LoadImage(AfxGetInstanceHandle(),
			MAKEINTRESOURCE(IDI_TEXT_STYLE),
			IMAGE_ICON,
			0,
			0,
			0);
	...
	if(TRUE == b)
	{
		...
		m_wndGridCtrl.SetColumnWidth(1 , 180);

		m_wndGridCtrl.SetColumnWidth(0 , m_wndGridCtrl.GetColumnWidth(0) * 2);
	}
...
int CTextStyleDlg::UpdateContents()
{
	...
	for(int i = 0;i < oTextStyleResult.size();++i)
	{
