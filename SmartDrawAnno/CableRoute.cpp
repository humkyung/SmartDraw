...
CCableOutputData FindTagItemPositionForCableTray(vector<CaDraw_TagCableTray*>& oCableTrayList , CaDraw_View* pView , bool bTag)
{
	...
	int nCellHeight = int(ceil(nBoxHeight * CSmartDrawAnnoModule::m_pCellMap->scale())); 
	...
	///const double nItemCellHeight   = env.m_cableT.m_oLabelTextStyle.m_nHeight * CSmartDrawAnnoModule::m_pCellMap->scale();
	...
	int nCellWidth = int(ceil(nBoxWidth_Result * CSmartDrawAnnoModule::m_pCellMap->scale())); //Box의 width를 Cell 단위로 변환

	m_cen = Ci_iter->m_ptCenter; /// Cable Tray의 Center           
	m_len = Ci_iter->length();  /// Cable Tray의 Length	
	//W_cnt = 0;
	m_cnt = int(ceil(m_len * CSmartDrawAnnoModule::m_pCellMap->scale())); //Cable Tray의 Length를 Cell 단위로 변환
	if (0 == m_cnt) m_cnt = 2;
	PR_cnt = 0;   
	...
	CCellMap::CellIndex ins_index = CSmartDrawAnnoModule::m_pCellMap->PointToIndex(m_cen); //!  그래픽 좌표를 CELL 좌표로 변환시킨다.
	//m_Cnt
	...
	st_x = (st_x < 0) ? 0 : st_x;
	ed_x = (ed_x >= (int(CSmartDrawAnnoModule::m_pCellMap->xlen()) - nCellWidth)) ? ((CSmartDrawAnnoModule::m_pCellMap->xlen() - nCellWidth) - 1) : ed_x;
	st_y = (st_y < 0) ? 0 : st_y;
	ed_y = (ed_y >= (int(CSmartDrawAnnoModule::m_pCellMap->ylen()) - nCellHeight))? ((CSmartDrawAnnoModule::m_pCellMap->ylen() - nCellHeight)- 1) : ed_y;
	//!
	...
	{
		int K = 0;
		if (!(st_x < 0) && !(ed_x >= int(CSmartDrawAnnoModule::m_pCellMap->xlen())))
		{
			if ((ed_y - st_y) > 0) 
			{
				for (int J = st_y; J <= ed_y; ++J)
				{
					for (int I = st_x; I <= ed_x; ++I)
					{
						...
						for (int II = I; II <= (I + nCellWidth -1); ++II)
						{
							...
							{
								if (CSmartDrawAnnoModule::m_pCellMap->Value(II , JJ) != 0) K++;
							}
						}
						...
						cIndex.x = I;
						cIndex.y = J;
						CIsPoint2d tag_po = CSmartDrawAnnoModule::m_pCellMap->IndexToPoint(cIndex);
						oMidData.cnt = K;
	...
	if (PR_cnt > 0)
	{
		...
		else if (_T("REDUCE") == res.m_sType)
		{
			...
			res.m_oReducePntList.insert(res.m_oReducePntList.end() , Ci_iter->m_oReducePntList.begin() , Ci_iter->m_oReducePntList.end());
		}
		...
		if (bTag)
		{
			...
			CSmartDrawAnnoModule::m_pCellMap->UpdateRectOf(minPnt , maxPnt , 13);
		}
...
void CCableOutputData::WriteTrayTag(OFSTREAM_T& ofile)
{
	...
	ofile << _T("<GROUP>|") << (g_GroupNumber++) << _T(",CABLET^Pat 00") << std::endl;
...
int CCableTrayRoute::SetCableHatchData(CaDraw_View* pView , vector<CCableOutputData>& CableHatchOutputList)
{
	...
	for(list<CaDraw_TagCableTray*>::iterator itr = m_oCableTrayList.begin(); itr != m_oCableTrayList.end();++itr)
	{
		...
		else if (_T("REDUCE") == oOutputData.m_sType)
		{
			oOutputData.m_sSubType       = (*itr)->m_sSubType;
			oOutputData.m_nWidth2        = (*itr)->m_nWidth2;
			oOutputData.m_oReducePntList.insert(oOutputData.m_oReducePntList.begin() , (*itr)->m_oReducePntList.begin() , (*itr)->m_oReducePntList.end());
		}
...
{
	CaDraw_TagEnv& env = CaDraw_TagEnv::GetInstance();
	const double vScale = CSmartDrawAnnoModule::m_pCellMap->scale();
	...
	for(list<CaDraw_TagCableTray*>::iterator itr = m_oCableTrayList.begin(); itr != m_oCableTrayList.end();++itr)
	{
		...
		CCellMap::CellIndex cIndex = CSmartDrawAnnoModule::m_pCellMap->PointToIndex(m_cen); //!  그래픽 좌표를 CELL 좌표로 변환시킨다.
		//m_Cnt
			...
			if ((ed_y - st_y) > 0) 
			{
				for (int J = st_y; J <= ed_y; ++J)
				{
					for (int I = st_x; I <= ed_x; ++I)
					{
						...
						tag_po = CSmartDrawAnnoModule::m_pCellMap->IndexToPoint(CCellMap::CellIndex(I , J));
						oMidData.cnt = K;
						oMidData.PRI(tag_po, nBoxWidth_Result , nBoxHeight);
						oMidDataList.push_back(oMidData);				
						PR_cnt++;
...
int CCableTrayRoute::CableRoute_Sort()
{
	...
	do
	{
		...
		{	
			bLoop = false;
			nCnt = 0;
			for(vector<CIsPoint3d>::iterator jtr = (*itr)->m_oConnPointList.begin();jtr != (*itr)->m_oConnPointList.end();++jtr)
			{
				...
				if(frontPnt == (*jtr)) 
				{
					...
					else		frontPnt = *((*itr)->m_oConnPointList.begin());
		...
		{	
			...
			for(vector<CIsPoint3d>::iterator jtr = (*itr)->m_oConnPointList.begin();jtr != (*itr)->m_oConnPointList.end();++jtr)
			{
				...
				if(backPnt == (*jtr)) 
				{
					...
					else		backPnt = *((*itr)->m_oConnPointList.begin());
